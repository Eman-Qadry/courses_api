...
module.exports = {
  addVideosValidation: [
    body("videos").isArray({ min: 1 }).withMessage("Videos must be a non-empty array."),
    body("videos.*.url").isURL().withMessage("Each video must have a valid URL."),
    body("videos.*.topicName").isString().notEmpty().withMessage("Each video must have a topic name."),
  ],
